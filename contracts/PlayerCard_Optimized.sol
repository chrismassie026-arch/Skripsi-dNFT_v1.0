...
// Library renderer SVG, memisahkan logika rendering SVG dari kontrak utama
library PlayerCardRenderer {
    using Strings for uint256;

    // Layer 1: Latar belakang kartu (panel, border, dan teks header)
    ...
    // Layer 2: Sprite pixel art karakter pemain (32x32 piksel, di-hardcode sepenuhnya on-chain)
    function getPixelArtLayer() internal pure returns (string memory) {
        return '<svg x="10" y="30" width="140" height="380" viewBox="0 0 32 32" shape-rendering="crispEdges">...</svg>';
    }

    // Layer 3: Panel statistik sisi kanan kartu (nickname, lane, dan angka stats)
    function getStatsLayer(
        ...
        return string.concat(
            ...
            _getStatsRows(d, kdaStr, labelColor),
            '</g>'
        );
    }

    // Pembantu getStatsLayer: dipecah jadi 3 bagian string untuk menghindari error "stack too deep"
    function _getStatsRows(
        PlayerData memory d,
        string memory kdaStr,
        string memory labelColor
    ) internal pure returns (string memory) {
        // Bagian 1: Lane dan Games
        ...
        // Bagian 3: Assists, KDA, dan garis pemisah bawah
...
contract MplPlayerCard_Optimized is ERC721URIStorage, Ownable, FunctionsClient {
    using Strings for uint256;
    ...
    function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
        ...
        s_playerData[tokenId].assists = uint64(packedData);         // bits 0-63
        s_playerData[tokenId].deaths  = uint64(packedData >> 64);   // bits 64-127
        s_playerData[tokenId].kills   = uint64(packedData >> 128);  // bits 128-191
        s_playerData[tokenId].games   = uint64(packedData >> 192);  // bits 192-255
